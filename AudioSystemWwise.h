...
public:
	...
	virtual bool RegisterEntity(const CGameObject* entity, const char* name);
	virtual bool UnRegisterEntity(const CGameObject* entity);
	virtual bool RegisterListener(CGameObject* entity, const char* name);
	virtual bool UnRegisterListener(CGameObject* entity);
	
	//Posts an audio event at the caller's position
	virtual bool PostEvent(AudioEvent eventId,const CGameObject* eventCaller);
	...
	// Materials
	void SetMaterialID(const CGameObject* pEntity, AkUniqueID akMaterialSwitchDefine, const unsigned int nMaterialID);
	...
	void SetRTCPValue(const char* szRTCPName, const AkRtpcValue akRtcpVal, const CGameObject* pEntity);
	void SetRTCPValue(const AkRtpcID akRtcpID, const AkRtpcValue akRtcpVal);
	void SetRTCPValue(const AkRtpcID akRtcpID, const AkRtpcValue akRtcpVal, const CGameObject* pEntity);
	...
	// Switches
	void SetSwitch(const AkSwitchGroupID akSwitchGroupID, const AkSwitchStateID akStateID, const CGameObject* pEntity = nullptr);

	// Triggers
	void PostTrigger(const char* szTriggerName);
	void PostTrigger(const AkTriggerID akTriggerID);
	void PostTrigger(const char* szTriggerName, const CGameObject* pEntity);
	void PostTrigger(const AkTriggerID akTriggerID, const CGameObject* pEntity);
...
protected:
	...
	std::vector<CGameObject*> m_RegisteredListeners;		// entities registered as Listeners
	std::vector<const CGameObject*> m_RegisteredEntities;			// entities registered as game objects
